    ...
    public class PointGrassRendererEditor : Editor {
        ...
        private void DrawDistributionParameters() {
            ...
            if (distSource == DistributionSource.TerrainData) {
                ...
                // --- Helper for Scene Terrain Assignment ---
                if (prop_terrain.objectReferenceValue == null) {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.PrefixLabel("  target form scene:");
                    Terrain sceneTerrain = EditorGUILayout.ObjectField(null, typeof(Terrain), true) as Terrain;
                    if (sceneTerrain != null) {
                        if (sceneTerrain.terrainData != null) {
                            prop_terrain.objectReferenceValue = sceneTerrain.terrainData;
                        } else {
                            Debug.LogError("The assigned Terrain does not have any TerrainData connected!", sceneTerrain.gameObject);
                        }
                    }
                    EditorGUILayout.EndHorizontal();
                    if (prop_terrain.objectReferenceValue == null) {
                        EditorGUILayout.HelpBox("Drag your 'Terrain' GameObject from the Hierarchy into the slot above to auto-assign the Data.", MessageType.Info);
                    }
                }
                // -------------------------------------------
